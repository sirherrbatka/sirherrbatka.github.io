...
1. I work with a sparse datasets. Data frames should be sparse to not waste memory to store mostly empty columns.
2. Design should be layered in such a way that it becomes possible to quickly hack in needed features. For instance, It should be possible to construct data frames in a way simple enough to cooperate with other libraries.
