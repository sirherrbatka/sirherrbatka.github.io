...
Table itself is a mutable object composed of header and sequence of columns. AT generic function allows to access value in the individual cell of the table, but it is strongly advised to not loop over the table, and instead use TRANSFORM function for any transforming changes and a relevant aggregation function from the cl-data-structures package for aggregation (calculating sum of all elements in the column, finding minimum or maximum and so one).
* Data structures
...
* High level API
For the most typical tasks user is expected to simply stick to the high level API and do not dwell in the low level details described above. Therefore I've added separate package gathering relevant symbols from the layers below and rexport those from it. In addition, this package provides few extra generic functions defining basic serialization and deserialization protocol, as well as plain old functions for printing table in a human redable format to an output-stream.
* Use cases
* The future
