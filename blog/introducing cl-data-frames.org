...
We won't represent diagnosis as a string, we are afterall interested in list of ICD diagnosis. We are going to split strings representing diagnosis into individual codes during loading and also ensure that only a single instance of string is assigned to each code that needs to be represented. That's quite simple to do.
...
Once table is constructed analysis can be peformed. In this example we will count what is the number of diagnosis grouped around the diagnosis prefix and DRG. For instance, it wonders me if gooses are responsible for more hospitalizations then aggressive parrots (in some of the DRGs)!
